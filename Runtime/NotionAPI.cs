...
namespace BennyKok.NotionAPI
{
    public class NotionAPI
    {
        ...
        private readonly static string urlPage = rootUrl + "/pages";
        ...
        public IEnumerator PostJSON<T>(string url, Action<string> callback, T param)
        {
            if (debug) Debug.Log("POST Requesting: " + url);
            using (var request = WebRequestWithAuth(url, RequestType.POST, null))
            {
        ...
        public IEnumerator CreatePage<T>(T param, Action<string> callback)
        {
            yield return PostJSON(urlPage, callback, param);
        }
    }
}
